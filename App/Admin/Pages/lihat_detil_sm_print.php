...
<head>
  ...
  <title>SIPES | Detail Jumlah Surat Masuk</title>
  ...
  <section class="invoice">
    ...
    <div class="row">
      <div class="col-xs-12">
        ...
        <table width="100%">
            <tr>
                <td style="text-align: center; font-size: 20px;">JUMLAH JENIS SURAT MASUK <br> BULAN <?php 
                 switch ($bulan) {
            case '1':
              echo "JANUARI";
              break;
...
<?php 
  $query_sm = "SELECT `tb_jenis_surat`.`jenis_surat`, COUNT(*) AS tot_sm FROM `tb_surat_masuk` INNER JOIN `tb_jenis_surat` ON `tb_surat_masuk`.`id_jenis` = `tb_jenis_surat`.`id_jenis` WHERE YEAR(`tb_surat_masuk`.`tgl_terima`)=$tahun AND MONTH(`tb_surat_masuk`.`tgl_terima`) = $bulan GROUP BY `tb_surat_masuk`.`id_jenis`";
  ...
  while ($data_sm = mysqli_fetch_array($sql_sm)) {
    ...
    echo "<td style='padding-left: 10px;'>".$data_sm['jenis_surat']."</td>";
    echo "<td style='text-align:center;'>".$data_sm['tot_sm']."</td>";
    $tot_semua = $tot_semua + $data_sm['tot_sm'];
    echo "</tr>";
  }
